...
<div class="container mx-auto px-6 py-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        @if($products->isNotEmpty())
            @foreach($products as $product)
                <div class="border rounded-lg shadow hover:shadow-lg p-4 bg-white flex flex-col">
                    <!-- Product Image -->
                    <img src="{{ asset('storage/' . ltrim($product->Image, '/')) }}" 
                         alt="{{ $product->Name }}" 
                    ...
                    <h3 class="text-lg font-semibold text-gray-800">{{ $product->Name }}</h3>
                    <p class="text-gray-600 text-sm">{{ $product->Description }}</p>
                    <p class="text-blue-700 font-bold mt-2">Rs. {{ number_format($product->Price, 2) }}</p>

                    <!-- Livewire Add to Cart -->
                   @livewire('add-to-cart', ['productId' => $product->ProductId], key($product->ProductId))
